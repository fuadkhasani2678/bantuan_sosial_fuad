...
<body>
    ...
    <div class="container py-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1><i class="fas fa-user-edit text-primary"></i> Edit Penerima Bantuan</h1>
        ...
        <div class="card p-4">
            <form action="{{ route('penerima.update', $penerima) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nik" name="nik" value="{{ $penerima->nik }}" required>
                    @error('nik') <span class="text-danger">{{ $message }}</span> @enderror
                </div>

                <div class="mb-3">
                    <label for="nama" class="form-label"><i class="fas fa-user form-icon"></i> Nama</label>
                    <input type="text" class="form-control" id="nama" name="nama" value="{{ $penerima->nama }}" required>
                    @error('nama') <span class="text-danger">{{ $message }}</span> @enderror
                </div>

                <div class="mb-3">
                    <label for="tanggal_lahir" class="form-label"><i class="fas fa-calendar form-icon"></i> Tanggal Lahir</label>
                    <input type="date" class="form-control" id="tanggal_lahir" name="tanggal_lahir" value="{{ $penerima->tanggal_lahir }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="jenis_kelamin" name="jenis_kelamin" required>
                        ...
                        <option value="Laki-laki" {{ $penerima->jenis_kelamin == 'Laki-laki' ? 'selected' : '' }}>Laki-laki</option>
                        <option value="Perempuan" {{ $penerima->jenis_kelamin == 'Perempuan' ? 'selected' : '' }}>Perempuan</option>
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="alamat" name="alamat" rows="3" required>{{ $penerima->alamat }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    <select class="form-select" id="status_bantuan" name="status_bantuan" required>
                        <option value="">Pilih</option>
                        <option value="Aktif" {{ $penerima->status_bantuan == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="Tidak Aktif" {{ $penerima->status_bantuan == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                    </select>
